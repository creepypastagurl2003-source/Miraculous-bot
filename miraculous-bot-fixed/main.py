...
TOKEN = (
    os.environ.get("MIRACULOUS_BOT_TOKEN")
    or os.environ.get("DISCORD_BOT_TOKEN")
)
if not TOKEN:
    logger.error("No bot token found — set MIRACULOUS_BOT_TOKEN or DISCORD_BOT_TOKEN.")
...
async def on_disconnect():
    logger.warning("Gateway disconnected. discord.py will attempt to reconnect automatically.")


@bot.event
async def on_resumed():
    logger.info("Gateway session resumed — bot is fully back online.")
...
if __name__ == "__main__":
    ...
    if TOKEN:
        ...
        logger.error("Keep-alive running but Discord is offline — no token found.")
        import time
        while True:
            time.sleep(60)
