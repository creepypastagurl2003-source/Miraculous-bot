...
async def _safe_json(r: aiohttp.ClientResponse) -> Optional[dict]:
    """Return parsed JSON or None if the response is not valid JSON."""
    try:
...
async def resolve_youtube_channel(session: aiohttp.ClientSession, query: str) -> Optional[dict]:
    ...
    try:
        ...
        if re.match(r"^UC[A-Za-z0-9_-]{22}$", query):
            ...
            async with session.get(YT_CHANNEL_URL, params=params, timeout=API_TIMEOUT) as r:
                data = await _safe_json(r) or {}
        ...
        logger.warning(f"resolve_youtube_channel error for '{query}': {e}")
...
async def fetch_latest_video(session: aiohttp.ClientSession, channel_id: str) -> Optional[dict]:
    ...
    try:
        async with session.get(YT_SEARCH_URL, params=params, timeout=API_TIMEOUT) as r:
            if r.status != 200:
                return None
            data = await _safe_json(r) or {}
            items = data.get("items", [])
        ...
        logger.warning(f"fetch_latest_video error for channel {channel_id}: {e}")
        return None
...
async def get_twitch_token(session: aiohttp.ClientSession) -> Optional[str]:
    ...
    try:
        async with session.post(TWITCH_TOKEN_URL, params=params, timeout=API_TIMEOUT) as r:
            if r.status != 200:
                return None
            data = await _safe_json(r) or {}
        ...
        logger.warning(f"get_twitch_token error: {e}")
        return None
...
async def fetch_twitch_stream(session: aiohttp.ClientSession, username: str) -> Optional[dict]:
    ...
    try:
        async with session.get(TWITCH_STREAMS_URL, params={"user_login": username}, headers=headers, timeout=API_TIMEOUT) as r:
            if r.status == 401:
                global _twitch_token
                _twitch_token = None
                return None
        ...
        logger.warning(f"fetch_twitch_stream error for '{username}': {e}")
        return None
...
async def resolve_twitch_user(session: aiohttp.ClientSession, username: str) -> Optional[str]:
    ...
    headers = {"Client-ID": TWITCH_CLIENT_ID, "Authorization": f"Bearer {token}"}
    try:
        async with session.get(TWITCH_USERS_URL, params={"login": username}, headers=headers, timeout=API_TIMEOUT) as r:
            if r.status != 200:
                return None
            data = await _safe_json(r) or {}
            items = data.get("data", [])
        ...
        logger.warning(f"resolve_twitch_user error for '{username}': {e}")
        return None
